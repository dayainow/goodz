...
## v0.21에서 실제로 관리할 수 있는 것

Process Dashboard는 더 이상 상태만 보여주는 화면이 아닙니다. 사용자가 기본 P0–P4 Template으로 프로젝트를 시작하고 실행 상태를 직접 관리할 수 있습니다.
...
가변 실행 상태는 SQLite schema v2가 기준이며, PRD·설계·QA·릴리스 문서와 GitHub/CI 증거는 기존 Git SSOT를 유지합니다. 현재 제공 범위는 단일 사용자 Core와 기본 P0–P4 Template입니다. 사용자 정의 Template Builder, Deliverable/Evidence command, RBAC와 PostgreSQL은 다음 단계입니다.

## 새 프로젝트를 처음 시작한다면

현재 v0.21은 `goodz init` CLI 이전 단계입니다. 지금은 이 저장소를 템플릿으로 복제하고 조직·프로젝트 설정을 바꾸는 방식으로 시작합니다.
...
2. **프로젝트**에서 이름, 목표, Owner와 P0–P4 Template을 선택해 Process Run을 시작합니다.
