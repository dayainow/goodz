...
가변 실행 상태는 SQLite schema v4가 기준이며, 승인 전 PRD/Design 초안과 실행 상태를 저장합니다. 승인된 장문 문서와 GitHub/CI 원본은 기존 SSOT로 내보내 연결하는 구조를 유지합니다. 현재 제공 범위는 단일 사용자 Core, Visual Template Builder와 PRD/Design Workbench입니다. 원본 Git export, Claude Connector, RBAC와 PostgreSQL은 다음 단계입니다.

## 새 프로젝트를 처음 시작한다면

현재 v0.24는 `goodz init` CLI 이전 단계입니다. 지금은 이 저장소를 템플릿으로 복제하고 조직·프로젝트 설정을 바꾸는 방식으로 시작합니다.
