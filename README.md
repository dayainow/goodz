...
> **Lifecycle notice · 2026-07-15:** Goodz는 **[v1.1.0](https://github.com/dayainow/goodz/releases/tag/v1.1.0)** 검증 기준선에서 기능 동결하고 Reference 저장소로 마무리했습니다. 프로젝트 폴더에 설치되는 후속 로컬 Project OS는 **[Beacon](https://github.com/dayainow/beacon-project-os)**이며 별도 저장소에서 시작합니다. 종료 상태는 [ARCHIVE_HANDOFF](./docs/00-process/ARCHIVE_HANDOFF.md), 선별 이식 기준은 [TRANSFER_MANIFEST](./docs/00-process/TRANSFER_MANIFEST.md)를 확인하세요.
...
v1.1의 `@goodz/cli`는 사용자 저장소를 Goodz 자체 개발 이력 없이 빈 Workspace로 초기화합니다. 기존 코드에는 adopt를 적용하고, Dashboard에서 쌓은 승인 산출물은 프로젝트 저장소의 Markdown으로 materialize합니다.

### 1. 저장소 준비

```bash
pnpm goodz -- init --name "새 제품명" --root /path/to/new-project
```

현재 명령은 Goodz 소스 checkout에서 실행합니다. `@goodz/cli` registry 배포 후에는 대상 저장소에서 `pnpm dlx @goodz/cli init ...`으로 같은 설치를 수행합니다. 생성되는 `goodz.config.json`은 Reference 0개, `.goodz/workspace.json`은 독립 SQLite 경로를 가지며 Goodz의 Sprint·IN·CR·TL 이력은 복사하지 않습니다.
...
| 순서 | 수정 대상 | 결정할 내용 |
|---|---|---|
| 1 | `goodz.config.json` | edition, Core 설정, 선택적 Reference |
| 2 | `.goodz/workspace.json` | Workspace 식별자와 독립 DB 경로 |
| 3 | `docs/00-process/` | 사용자 프로젝트의 운영 기준 |
| 4 | `docs/projects/` | 승인 후 생성되는 PRD·Design Pack·handoff |
| 5 | Reference | 필요한 경우에만 자체 도메인 Reference 추가 |
...
각 단계의 Task·산출물·Evidence·Gate는 프로젝트별 Operations DB와 감사 이벤트에 누적합니다. `references/goodz-internal/status.json`은 Goodz 제품 개발자만 사용하는 별도 내부 Reference이며 사용자 프로젝트가 수정하지 않습니다. 상세 절차는 [Goodz Core Onboarding](./docs/00-process/ONBOARDING.md)과 [WORKFLOW](./docs/00-process/WORKFLOW.md)을 따릅니다.
...
| **Goodz Reference** | Goodz 자체 업그레이드에 사용한 IN·CR·DR·TL·Sprint·지표. 내부 설정이 있을 때만 표시 | `references/goodz-internal/` |
