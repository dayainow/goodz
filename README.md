...
가변 실행 상태는 SQLite schema v5가 기준이며, 승인 전 PRD/Design 초안과 Design Job 실행 상태를 저장합니다. 승인 결과는 경로와 Markdown 콘텐츠를 가진 portable bundle로 내보냅니다. 현재 Connector는 Claude Design UI를 사용하는 수동 어댑터이며, 자동 MCP/API 호출, Git commit export, RBAC와 PostgreSQL은 다음 단계입니다.

## 새 프로젝트를 처음 시작한다면

현재 v0.25는 `goodz init` CLI 이전 단계입니다. 지금은 이 저장소를 템플릿으로 복제하고 조직·프로젝트 설정을 바꾸는 방식으로 시작합니다.
