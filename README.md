...
가변 실행 상태는 SQLite schema v5가 기준이며, 승인 전 PRD/Design 초안과 Design Job 실행 상태를 저장합니다. 승인 결과는 portable bundle로 반환되고 Goodz CLI가 이를 충돌 보호된 Git-ready Markdown으로 생성합니다. 기존 모노레포는 읽기 전용 adopt 계획 후 적용하고, Template 변경은 기존 Run을 건드리지 않는 새 버전으로 생성합니다. 자동 Git commit/PR, Claude MCP/API 호출, RBAC와 PostgreSQL은 다음 단계입니다.

## 새 프로젝트를 처음 시작한다면

v0.28부터 `@goodz/cli`가 초기화·기존 저장소 adopt·Template migration·프로젝트 생성·산출물 export·무결성 검증을 제공합니다. 애플리케이션 scaffold 전체 복사는 아직 Goodz 저장소 Template을 사용합니다.
...
| Goodz CLI | 🟢 init·adopt·template migrate·project create·safe export·verify + hash manifest |
