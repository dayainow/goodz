...
| Template Builder | Dashboard의 JSON Builder로 Stage·Task·산출물 정의를 저장하고 즉시 실행 |

가변 실행 상태는 SQLite schema v3가 기준이며, PRD·설계·QA·릴리스 문서와 GitHub/CI 원본은 기존 SSOT를 유지합니다. 현재 제공 범위는 단일 사용자 Core, 기본 Template 2종과 JSON Template Builder입니다. 시각적 Builder, RBAC와 PostgreSQL은 다음 단계입니다.

## 새 프로젝트를 처음 시작한다면

현재 v0.22는 `goodz init` CLI 이전 단계입니다. 지금은 이 저장소를 템플릿으로 복제하고 조직·프로젝트 설정을 바꾸는 방식으로 시작합니다.
