...
| **Goodz Internal Service Reference** | 비커머스 이식성과 독립 계약을 증명하는 예제 | `@goodz/internal-service-types`, Node service catalog API |
...
```text
 ...
 @goodz/internal-service-types ──> service catalog API
              Core changes: 0 / Commerce dependencies: 0
```
...
@goodz/ui      → UI 소비 앱
@goodz/internal-service-types → internal-service API
```
...
- `pnpm check:portability`는 Core 계약 hash와 두 번째 Reference의 독립성을 검증한다.
...
- **이식성 계약** — `goodz.config.json#portability`, `pnpm check:portability`
- **전체 검증** — `pnpm verify`
...
| `references/internal-service/packages/types` | 비커머스 Reference 타입 SSOT |
| `references/internal-service/apps/api` | 서비스 카탈로그 Reference API |
...
1. ✅ Internal Service Reference로 Core 수정 0건 이식성을 검증했다.
