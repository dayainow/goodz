...
- [x] config v1→v2 dry-run·원자적 migration·멱등성이 동작함
- [x] clean-clone에서 offline frozen install·CLI·Template·Portability 검증이 통과함
- [ ] clean clone에서 설치·마이그레이션·전체 검증이 통과함

위 항목으로 Goodz Core v1.0 설치·이식성 Gate를 충족한다. Enterprise-ready 표시는 SSO/RBAC·감사·정책 팩·백업/복구 Gate 이후 별도로 결정한다.

## Repository Closure Gate

Goodz를 v1.1 Reference 기준선으로 종료하고 후속 제품을 별도 저장소로 시작하기 위한 Gate다.

- [x] 기능 동결과 저장소 분리 결정이 `DEC-029`에 기록됨
- [x] 후속 제품명이 Beacon으로 확정되고 `DEC-030`에 기록됨
- [x] 후속 제품 목표·MVP·비목표가 `PRD-SUCCESSOR-SPINOUT.md`에 기록됨
- [x] 이식·제외 대상이 `TRANSFER_MANIFEST.md`에 경로 단위로 기록됨
- [x] Goodz 자체 기록과 빈 사용자 Workspace의 런타임 경계가 구현됨
- [x] 빈 Workspace HTTP smoke가 통과함
- [x] Reference 없는 Dashboard 로컬 육안 QA 통과
- [x] 최종 `pnpm verify` 통과
- [x] Beacon 공개 빈 저장소와 원격 URL 확인
- [x] README와 인계 문서에 Beacon 저장소 링크 추가
- [ ] 미커밋 변경을 보존하고 Cursor가 종료 commit을 생성함
