...
| DEC-024 | 2026-07-14 | 저장소 파일 쓰기는 API와 분리된 CLI가 담당하고 hash manifest로 로컬 수정을 보호 | CR-025, TL-025 |
| DEC-025 | 2026-07-14 | 기존 저장소 adopt는 read-only 계획을 기본으로 하고 명시적 apply에서만 설정을 생성 | CR-026, TL-026 |
| DEC-026 | 2026-07-14 | Template 변경은 새 immutable version으로 만들고 기존 Run은 생성 시점 version에 고정 | CR-027, TL-027 |
| DEC-027 | 2026-07-14 | Git 자동화는 CLI 권한으로 승인 export allowlist만 branch·commit·push·PR 처리 | CR-028, TL-028 |
| DEC-028 | 2026-07-14 | Core v1.0은 config migration·clean-clone·publish metadata로 설치 계약을 고정 | CR-029, TL-029 |
| DEC-029 | 2026-07-15 | Goodz를 v1.1 기준선에서 기능 동결하고 로컬 Project OS를 별도 저장소로 분리 | Successor Spin-out PRD |
| DEC-030 | 2026-07-15 | 후속 로컬 Project OS의 제품명을 Beacon으로 확정 | Beacon Spin-out PRD |
| DEC-031 | 2026-07-15 | Beacon 저장소는 초기 MVP 동안 비공개로 운영 | DEC-032로 대체 |
| DEC-032 | 2026-07-15 | Beacon 저장소를 공개 빈 저장소로 시작 | Archive Handoff |
| DEC-033 | 2026-07-15 | Goodz를 Beacon과 무관하게 Usability Closure로 재개 — 메인 경로(프로젝트→Task→Gate) 우선, Archive 동결 해제 | PROJECT.md, NORTH_STAR |
| DEC-034 | 2026-07-15 | 프로젝트 시작·Gate GO 시 docs/projects 스캐폴드, 완료 시 Project Book으로 결과물을 남긴다 | API.md, Usability Closure |

## DEC-034 — Artifact Scaffold와 Project Book

### 배경

Control Plane만으로는 실행 결과가 남지 않아 서비스가 허무하게 느껴졌다.

### 결정

1. 프로젝트 시작과 Stage GO 때 `docs/projects/<id>/`에 Markdown을 남긴다.
2. Run 여정은 Project Book(`GET /book` + `PROJECT_BOOK.md`)으로 export한다.
3. `:memory:` DB·`GOODZ_SKIP_ARTIFACT_WRITE=1`에서는 디스크 쓰기를 건너뛴다.

## DEC-033 — Goodz Usability Closure

### 배경

v1.1 Archive(DEC-029) 이후 Dashboard에 Template·PRD·Reference·Metrics가 한꺼번에 노출되어 “뭘 하면 되는지”가 불명확했습니다. Beacon 이관과 별개로 Goodz 자체를 목적에 맞게 쓸 수 있게 마무리할 필요가 생겼습니다.

### 결정

1. Goodz 저장소에서 **기능 동결을 해제**하고 Usability Closure 스프린트를 진행한다.
2. 제품 메인 경로는 **프로젝트 생성 → Stage Task → 산출물 승인 → GO/HOLD/KILL**이다.
3. PRD/Design Workbench·Template Builder·Goodz Reference는 **선택/고급/관리자**로 접어 둔다.
4. Beacon은 후속 선택지이며, Goodz 마무리의 전제 조건이 아니다.

### 결과

- Process Dashboard Workspace UX를 메인 경로 중심으로 재배치
- NORTH_STAR · PROJECT · USER_MANUAL · ARCHIVE_HANDOFF 상태 문구 갱신
...
- `pnpm snapshot:metrics`로 현재 Delivery Metrics를 `references/goodz-internal/metrics-snapshots.json`에 저장합니다.
...
## DEC-029 — 후속 로컬 Project OS 저장소 분리

### 배경

Goodz는 모노레포 스캐폴드, Process Gate, CLI, Dashboard, Commerce와 비커머스 Reference까지 한 저장소에서 검증했다. 기능을 순차적으로 누적하는 동안 사용자 프로젝트 운영과 Goodz 자체 운영, Reference와 제품 runtime의 경계가 복잡해졌다. 후속 제품은 별도 서비스에 프로젝트를 다시 등록하는 Control Plane보다 프로젝트 폴더에 설치되는 로컬 Project OS를 목표로 한다.

### 결정

- Goodz는 v1.1 Installable Empty Workspace를 최종 기능 기준선으로 동결한다.
- 후속 제품은 새 이름, 새 저장소와 새 Git history로 시작한다.
- 프로젝트 폴더 하나를 로컬 제품의 프로젝트 하나로 취급한다.
- Goodz 전체를 복사하지 않고 `TRANSFER_MANIFEST.md`의 승인 단위만 이식한다.
- Commerce, Internal Reference, 현재 Dashboard 전체, Template Builder와 고급 운영 기능은 MVP에 이식하지 않는다.

### 영향

- Goodz는 실패한 제품이 아니라 검증 완료된 Reference와 코드 원천으로 보존된다.
- 후속 제품은 `beacon init → beacon open → identity` 사용자 흐름부터 작게 시작할 수 있다.
- 기존 기능이 자동으로 후속 백로그가 되지 않아 초기 복잡도와 범위가 제한된다.
- 저장소 생성과 GitHub push 전에 Goodz Repository Closure Gate를 통과해야 한다.

## DEC-030 — 후속 제품명 Beacon 확정

### 배경

후속 제품은 프로젝트 여정 자체보다 진행 중인 프로젝트가 어느 방향으로 가야 하는지 보여주고, 부족한 부분과 다음 행동을 신호로 전달하는 역할이 핵심이다. Harbor는 완료와 보관, Odyssey는 전체 여정의 의미가 강한 반면 Beacon은 제품이 제공하는 안내 역할을 가장 직접적으로 설명한다.

### 결정

- 후속 로컬 Project Navigation OS의 표시 이름을 **Beacon(비콘)**으로 확정한다.
- 한 줄 정의는 “프로젝트 폴더에서 시작해 현재 방향, 부족한 부분, 산출물과 변경 이력을 보여주는 로컬 우선 Project Navigation OS”로 한다.
- 브랜드 서사에는 항해와 신호 메타포를 사용하되 핵심 UI 용어는 직관적인 프로젝트 용어를 유지한다.
- 동명 소프트웨어와의 기술적 혼동을 줄이기 위해 권장 저장소 slug는 `beacon-project-os`로 한다.
- CLI 명령 `beacon`과 로컬 상태 디렉터리 `.beacon/`은 새 저장소 P0에서 충돌과 배포 방식을 검증한 뒤 계약으로 고정한다.

### 영향

- 이름 확정으로 Beacon 저장소 생성과 P0 정보 구조 설계를 시작할 수 있다.
- 제품 표시명과 저장소·package 식별자를 분리해 공개 패키지 충돌에 대응한다.
- Goodz 문서에서는 일반적인 “후속 제품” 표현보다 Beacon을 우선 사용한다.

## DEC-031 — Beacon 저장소 초기 공개 범위

> **상태:** DEC-032로 대체됨.

### 배경

Beacon은 Goodz에서 필요한 코드만 선별 이식하고 제품 계약을 다시 세우는 초기 단계다. 저장소를 생성하는 시점에는 CLI·config·package scope와 라이선스가 아직 고정되지 않았고, 동명 소프트웨어와의 식별 방식도 검토가 필요하다.

### 결정

- `dayainow/beacon-project-os`는 비공개 저장소로 시작한다.
- Goodz 전체 소스나 Git history를 복사하지 않고 독립적인 초기 commit을 만든다.
- 공개 전 P0 Gate에서 제품 설명, 라이선스, package/CLI 식별자와 이식 출처 ADR을 검토한다.

### 영향

- 초기 MVP 설계와 선별 이식을 외부 호환성 약속 없이 수정할 수 있다.
- 공개 전 체크리스트를 통과할 때까지 README, package publish와 Release를 외부 배포로 간주하지 않는다.

## DEC-032 — Beacon 공개 저장소 전환

### 배경

초기에는 제품 계약과 라이선스가 정리되기 전까지 비공개 저장소를 권장했다. 사용자는 Beacon의 개발 과정을 공개해도 괜찮다고 결정했고, `dayainow/beacon-project-os`를 내용 없는 공개 저장소로 생성했다.

### 결정

- Beacon의 공식 저장소는 [dayainow/beacon-project-os](https://github.com/dayainow/beacon-project-os)다.
- 저장소는 Public으로 운영한다.
- Goodz Repository Closure Gate가 닫히기 전에는 Goodz 소스나 Git history를 이식하지 않는다.
- 첫 commit에는 제품 정의, 비목표, 이식 출처 ADR과 최소 workspace 구조만 포함한다.

### 영향

- Beacon의 의사결정과 구현 과정이 처음부터 공개된다.
- 라이선스와 공개 package 계약을 확정하기 전까지 배포 가능한 제품으로 오인되지 않도록 README에 초기 개발 상태를 표시해야 한다.
- DEC-031의 비공개 운영 결정은 이 결정으로 대체된다.
